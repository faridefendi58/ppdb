...
class M_siswa extends CI_Model {
    ...
    public function field_data($type = 1, $photo = NULL, $more = [])
    {
        $this->data = array(
                      ...
                      'tgl_lahir'      => strtoupper(extract_date($this->input->post('tgl_lahir'))),
                      ...
                      'st_anak'        => strtoupper($this->input->post('st_anak')),
                      'alamat'         => strtoupper($this->input->post('alamat')),
                      'no_kk'          => strtoupper($this->input->post('no_kk')),
    ...
    public function find_all($criteria = '', $year)
    {
        if ($criteria == '')
        {
            return $this->db->query("SELECT s.id, 
                                            s.no_daftar,
                                            UPPER(s.nama) AS nama,
                                    	    UPPER(s.asal_sekolah) AS asal_sekolah, 
                                            s.diterima,
                                    	    UPPER(p1.prodi_nama) AS pilihan_1, 
                                    	    UPPER(p2.prodi_nama) AS pilihan_2,
                                            UPPER(p3.prodi_nama) AS pilihan_3,
                                            UPPER(p4.prodi_nama) AS pilihan_4,
                                            s.n_raport_1_5,
                                            s.no_ujian_smp 
                                     FROM siswa s
        ...
        else if ($criteria == 1)
        {
            return $this->db->query("SELECT s.id, 
                                            ...
                                            UPPER(p4.prodi_nama) AS pilihan_4,
                                    		UPPER(s.nama) AS nama, 
                                    		UPPER(p5.prodi_nama) AS diterima,
                                    		s.n_raport_1_5,
                                            s.no_ujian_smp 
                                     FROM siswa s
                                     INNER JOIN prodi p1 ON p1.prodi_id = s.pil_1
        ...
        else if ($criteria == 3)
        {
            return $this->db->query("SELECT id,
                                            no_daftar,
                                            UPPER(nama) AS nama,
                                            UPPER(asal_sekolah) AS asal_sekolah,
                                            n_raport_1_5,
                                            no_ujian_smp 
                                     FROM siswa
                                     WHERE substring(no_daftar, 1, 4) = '$year'
    ...
    public function find($id)
    {
        $query = $this->db->query("SELECT s.id,
                                          ...
                                          s.no_akte,
                                          UPPER(s.nama) AS nama,
                                          UPPER(s.tmp_lahir) AS tmp_lahir,
                                          s.tgl_lahir,
                                          s.jalur_id,
                                          s.jns_kelamin,
                                          s.agama,
                                          s.anak_ke,
                                          s.sdr_kandung,
                                          UPPER(s.st_anak) AS st_anak,
                                          UPPER(s.alamat) AS alamat,
                                          s.no_kk,
                                          s.telp,
                                          s.no_kip,
                                          UPPER(s.email) AS email,
                                          s.b_badan,
                                          s.t_badan,
                                          s.g_darah,
                                          UPPER(s.asal_sekolah) AS asal_sekolah,
                                          UPPER(s.nama_ortu) AS nama_ortu,
                                          s.nik_ayah,
                                          UPPER(s.nama_ibu) AS nama_ibu,
                                          s.nik_ibu,
                                          UPPER(s.alamat_ibu) AS alamat_ibu,
                                          s.hasil_ibu,
                                          s.hp_ibu,
                                          UPPER(s.alamat_ortu) AS alamat_ortu,
                                          UPPER(pekerjaan.pk_nama) AS pk_nama,
                                          s.kerja_ibu,
                                          UPPER(pk2.pk_nama) AS pk_ibu_nama,
                                          s.hasil_ortu,
                                          s.telp_ortu,
                                          s.n_bindo,
                                          s.n_bing,
    ...
    {
        $sql = "SELECT s.id, s.no_daftar, s.tgl_daftar, s.nisn, s.nik, s.no_akte, UPPER(s.nama) AS nama,
            s.tmp_lahir, s.tgl_lahir, s.jalur_id, s.jns_kelamin, s.agama, s.anak_ke,
            s.sdr_kandung, s.st_anak, UPPER(s.alamat) AS alamat, s.no_kk, s.telp, s.no_kip, UPPER(s.email) AS email,
            s.b_badan, s.t_badan, s.g_darah, UPPER(s.asal_sekolah) AS asal_sekolah, UPPER(s.nama_ortu) AS nama_ortu, s.nik_ayah,
            UPPER(s.nama_ibu) AS nama_ibu, s.nik_ibu, UPPER(s.alamat_ibu) AS alamat_ibu, s.hasil_ibu, s.hp_ibu, UPPER(s.alamat_ortu) AS alamat_ortu,
            UPPER(pekerjaan.pk_nama) AS pk_nama, s.kerja_ibu, UPPER(pk2.pk_nama) AS pk_ibu_nama, s.hasil_ortu, s.telp_ortu, s.n_bindo, s.n_bing,
            ...
            UPPER(p1.prodi_nama) AS pilihan_1, UPPER(p2.prodi_nama) AS pilihan_2, UPPER(p3.prodi_nama) AS pilihan_3, UPPER(p4.prodi_nama) AS pilihan_4			 
            FROM siswa s
